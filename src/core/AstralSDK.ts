...
export class AstralSDK {
  ...
  async buildLocationProof(input: LocationProofInput): Promise<UnsignedLocationProof> {
    ...
    try {
      ...
      if (!extension) {
        throw new ExtensionError(`No extension found for location format: ${locationType}`, undefined, {
          locationType,
          availableExtensions: this.extensions.getAllLocationExtensions().map(ext => ext.locationType)
        });
      }
      ...
      if (!locationExtension) {
        throw new ExtensionError(`No extension found for processed location format: ${finalLocationType}`);
      }
        ...
        for (const mediaItem of input.media) {
          ...
          if (!mediaExtension) {
            throw new ExtensionError(`No extension found for media type: ${mediaType}`, undefined, {
              mediaType,
              availableExtensions: this.extensions.getAllMediaExtensions()
      ...
      const unsignedProof: UnsignedLocationProof = {
        eventTimestamp: input.timestamp ? Math.floor(input.timestamp.getTime() / 1000) : Math.floor(Date.now() / 1000),
  ...
  async createOnchainLocationProof(input: LocationProofInput): Promise<UnsignedLocationProof> {
    ...
    if (this.debug) {
      console.log('Created unsigned location proof (registration not yet implemented):', unsignedProof);
    }
