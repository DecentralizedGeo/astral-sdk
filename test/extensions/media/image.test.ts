...
import {
  ...
  ensureDataUrl,
  stripDataUrlPrefix,
} from '../../../src/extensions/media/builtins/image';
...
describe('ImageExtension', () => {
  ...
  test('processMedia should process valid images', () => {
    ...
    expect(() => extension.processMedia('image/png', validJpegBase64)).toThrow(
      MediaValidationError
    );
    expect(() => extension.processMedia('image/gif', validJpegBase64)).toThrow(
      MediaValidationError
    );
  });
});
...
describe('Media Extensions Integration', () => {
  test('getMediaExtension should find the correct extension', () => {
    ...
    // Should return undefined for unsupported types (note: in original tests, only image was supported)
    expect(getMediaExtension('image/gif')).toBeUndefined();

    // Now we support video/mp4 and application/pdf with the new extensions
    expect(getMediaExtension('video/mp4')).toBeDefined();
    expect(getMediaExtension('application/pdf')).toBeDefined();
  });
  ...
  test('validateMediaData should use the correct extension', () => {
    ...
    // These should no longer throw since we support these media types
    // The current MVP implementation only does basic validation, which passes for any string data
    // In a future version, these would return false for incorrect data
    expect(validateMediaData('video/mp4', validJpegBase64)).toBe(true);
    expect(validateMediaData('application/pdf', validJpegBase64)).toBe(true);
  });
});
