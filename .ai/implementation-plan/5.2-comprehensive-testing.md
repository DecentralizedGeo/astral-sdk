...
1. **Schema Field Mismatch**: EAS-config.json uses singular field names (`recipeType`, `mediaType`) but TypeScript types use plural (`recipeTypes`, `mediaTypes`)
...
3. **Error Type Issues**: Some tests expect `SigningError` but get `EASError`
