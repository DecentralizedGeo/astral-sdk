...
- *Tasks*:
  ...
  #### **Sub-task 1A: Fix Schema Field Mismatch** (Highest Impact - resolves ~8 failures)
  - [ ] **Decision**: Update TypeScript types to use singular field names to match onchain schema (source of truth)
  - [ ] Change `recipeTypes` → `recipeType` in types.ts UnsignedLocationProof interface
  - [ ] Change `mediaTypes` → `mediaType` in types.ts UnsignedLocationProof interface
  - [ ] Update all code references: `recipeTypes` → `recipeType`, `mediaTypes` → `mediaType` 
  - [ ] Update AstralSDK.buildLocationProof to use singular field names
  - [ ] Update test expectations to match singular field names
  - [ ] Verify schema validation now passes in buildLocationProof method
  - [ ] **Commit checkpoint**: `fix(schema): align TypeScript types with onchain schema field names (singular)`
  
  #### **Sub-task 1B: Fix Async Extension Loading Race Condition** (resolves extension errors)
  ...
  #### **Sub-task 1C: Fix Error Type Alignment** (resolves error expectation mismatches)
  ...
  #### **Sub-task 1D: Fix API Client Issues** (resolves API test failures)
  ...
  #### **Sub-task 1E: Fix Signature Verification Logic** (resolves verification test failures)
