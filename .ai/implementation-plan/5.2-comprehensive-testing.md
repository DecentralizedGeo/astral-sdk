...
- *Tasks*:
  ...
  - [ ] **Fix Implementation**:
    - [ ] Debug actual EAS SDK transaction and receipt structure
    - [ ] Implement proper receipt parsing for EAS SDK responses
    - [ ] Extract real UID from transaction logs (EAS attestation events)
    - [ ] Extract real txHash and blockNumber from receipt
    - [ ] Add proper error handling if transaction metadata cannot be extracted
  - [ ] **Quality Gate**: Real transaction data must be returned from successful attestations
  - [ ] **Commit checkpoint**: `fix(onchain): extract real transaction metadata from EAS SDK responses`

  #### **Sub-task 1C: Fix Async Extension Loading Race Condition** (resolves extension errors)
  - [ ] **Decision**: Make extension loading synchronous or add proper async initialization
  - [ ] Option A: Convert extension imports to synchronous imports
  - [ ] Option B: Add `await ensureExtensionsInitialized()` method to AstralSDK
  - [ ] Implement chosen approach in ExtensionRegistry
  - [ ] Update AstralSDK constructor to handle extension initialization properly
  - [ ] **Commit checkpoint**: `fix(extensions): resolve async loading race condition`
  
  #### **Sub-task 1D: Fix Error Type Alignment** (resolves error expectation mismatches)
  - [ ] **IDENTIFIED ISSUES**:
    - [ ] OffchainSigner test expects `SigningError` but gets `EASError` 
    - [ ] createOnchainLocationProof test needs to expect `ValidationError` for missing provider
  - [ ] Review OffchainSigner error throwing - prefer EAS SDK or ethers error types where reasonable
  - [ ] Minimal refactoring: update test expectations to match actual error types thrown
  - [ ] Only create custom error types where EAS/ethers don't provide appropriate ones
  - [ ] **Commit checkpoint**: `fix(errors): align error expectations with actual SDK behavior`
  
  #### **Sub-task 1E: Fix API Client Issues** (resolves API test failures)
  - [ ] Fix URL encoding in AstralApiClient query parameter building
  - [ ] Handle bbox parameter encoding correctly (no URL encoding for comma-separated values)
  - [ ] Fix offset parameter - should default to 0 when not provided
  - [ ] Add proper timeout handling for network error tests
  - [ ] **Commit checkpoint**: `fix(api): resolve URL encoding and parameter handling issues`
  
  #### **Sub-task 1F: Fix Signature Verification Logic** (resolves verification test failures)
  - [ ] **IDENTIFIED ISSUE**: Invalid signatures being marked as valid (test expects false, gets true)
  - [ ] Debug OffchainSigner.verifyOffchainLocationProof method
  - [ ] Ensure invalid signatures are properly detected and marked as invalid
  - [ ] Verify EIP-712 signature validation logic is correct
  - [ ] **Commit checkpoint**: `fix(verification): ensure invalid signatures are properly rejected`
  
  - [ ] **Quality Gate**: All existing tests must pass before proceeding
  - [ ] **Completion Criteria**: `pnpm test` shows 0 failing tests (down from 14)
  - [ ] **Final Commit**: `test: complete resolution of all existing test suite failures`
