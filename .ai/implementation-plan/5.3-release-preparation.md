...
- *Tasks*:
  ...
  - [ ] Prepend copyright notice to LICENSE: `Copyright © 2025 Sophia Systems Corporation`
  - [ ] Update package.json license field from "MIT" to "Apache-2.0"
  - [ ] Update README.md license section from "MIT" to "Apache 2.0"
  - [ ] Update README "License" section to name Sophia Systems Corporation as maintainer
  - [ ] **COMMIT**: `legal: update license from MIT to Apache 2.0`
  
  #### **Sub-task 1B: License Header Implementation**
  - [ ] Add SPDX header to all source files: `// SPDX-License-Identifier: Apache-2.0`
  - [ ] Add copyright notice header: `// Copyright © 2025 Sophia Systems Corporation`
  - [ ] Create script to automate header addition to all TypeScript files
  - [ ] Verify all source files have proper headers (src/**/*.ts, test/**/*.ts)
  - [ ] **COMMIT**: `legal: add Apache 2.0 headers to all source files`
  
  #### **Sub-task 1C: IP Assignment and Corporate Ownership**
  - [ ] Draft IP Assignment Agreement (John Hoopes ➜ <C-Corp Name>)
  - [ ] Sign agreement and store PDF in `/legal` directory
  - [ ] Update npm org profile to list <C-Corp Name> as copyright holder
  - [ ] **COMMIT**: `legal: establish corporate ownership structure`
  
  #### **Sub-task 1D: Contributor License Agreement (CLA) Setup**
  - [ ] Upload CLA (individual + entity versions) naming <C-Corp Name> as assignee
  - [ ] Configure CLA-assistant on GitHub repository
  - [ ] Enable "Require PR signature" for default branch
  ...
  - [ ] Run `license-checker --json` to verify no GPL or restrictive licenses
  - [ ] Audit all dependencies for license compatibility
  - [ ] Document any license exceptions or clarifications
  - [ ] **COMMIT**: `legal: verify license compliance for all dependencies`
  
  - [ ] **Quality Gate**: All files have correct Apache 2.0 licensing and corporate ownership
  - [ ] **Completion Criteria**: Legal foundation established for corporate-owned open source project
  ...
  - [ ] Include license headers in all example files
